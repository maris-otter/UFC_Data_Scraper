...
def append_used_link(link):
    ...
    args:
        link - url to the page being tracks
    """
...
def organize_fight_data(fight_history_collection, http): #!!!!!Need to figure out how to use funcition. Return? call func()?
...
def diff_list(li1, li2):
    li_dif = set(li1) - set(li2)
    return li_dif

def update_fights():
    """
    Checks the difference in tracker.txt from the a tags containing a fight
    on http://www.ufcstats.com/statistics/events/completed. Every tag that is
    different is requested and saved to directory
    """
    url = 'http://www.ufcstats.com/statistics/events/completed?page=all'
    wanted_dir = 'ufc_scraper/src/test/temp'

    #if functio not being run within the directory listed above quit
    if not is_dir_correct(wanted_dir):
        exit("incorrect Directory")

    current_links = get_all_event_history_links()

    file_links = []
    #open the tracker file and append every line to a list
    try:
        with open('tracker.txt') as fp:
            line = fp.readline()
        ...
        print("An error occured while opening file. Unable to update.")
        return

    difference = diff_list(current_links, file_links)

    print("current links: " + str(len(current_links)))
    print("File links: " + str(len(file_links)))
    print("difference links: " + str(len(difference)))



    # if len(difference) == 0:
    #     print("File is still up to date. No changes")
    #     return
    #
    ...
    #
    # index = 0
    # for link in tqdm(difference): #tqdm is open source progress bar on for loop
    #     try:
    #         get_fight_history_http(requests_url = difference[index])
    #     except Exception:
    #         print("No links found in %s" % link)
    #         print("No completed fight links found. Might be an upcoming fight.")
    #         pass
